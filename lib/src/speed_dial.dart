    ...
    var child = widget.animatedIcon != null
        ...
        : AnimatedBuilder(
            ...
            builder: (BuildContext context, _) => Transform.rotate(
              angle: (widget.activeChild != null || widget.activeIcon != null) && widget.useRotationAnimation ? _controller.value * widget.animationAngle : 0,
      ...
      children: [
        Positioned(
          ...
          offset: widget.direction.isDown
              ? Offset((widget.switchLabelPosition || dialKey.globalPaintBounds == null ? 0 : dialKey.globalPaintBounds!.size.width) + max(widget.childrenButtonSize.height - 56, 0) / 2,
                  dialKey.globalPaintBounds!.size.height)
              : widget.direction.isUp
                  ? Offset((widget.switchLabelPosition || dialKey.globalPaintBounds == null ? 0 : dialKey.globalPaintBounds!.size.width) + max(widget.childrenButtonSize.width - 56, 0) / 2, 0)
                  : widget.direction.isLeft
...
Widget _buildColumnOrRow(bool isColumn, {CrossAxisAlignment? crossAxisAlignment, MainAxisAlignment? mainAxisAlignment, required List<Widget> children, MainAxisSize? mainAxisSize}) {
  return isColumn
      ? Column(
